...
const TransactionForm = ({ txn = null, onClose = null, onSuccess = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data || err.message || "Error saving transaction";

      // ✅ Handle insufficient balance with a confirmation popup
      if (errorMessage.includes("Insufficient balance")) {
        const userChoice = window.confirm(
          `${errorMessage}\n\nDo you want to adjust your amount?`
        );
        if (userChoice) {
          // ✅ User clicked OK → stay on the page so they can edit the amount
          return;
        } else {
          // ✅ User clicked Cancel → navigate back
